...
from typing import DefaultDict, Optional, Tuple, List, Dict, Any, get_type_hints
...
class Simulator:
    ...
    def simulate_disc(self, init_list, init_mode_list, agent_list, lane_map):

        disc_transitions = {}
        # Generate the controller graph for all agents individually
        for i, agent in enumerate(agent_list):

            # Initialize a MultiDiGraph for the agent            
            paths = [p for p in agent.decision_logic.paths]
            G = nx.MultiDiGraph()
            G_edge_id = 1
            G_edge_dict = dict()
            
            # Get a list of the agents's controller modes, if the agent has a controller
            if not agent.decision_logic.state_defs:
                print(f"Could not generate controller graph for agent {agent.id} as there is no controller.")
                continue
            parsed_modes = agent.decision_logic.state_defs["State"].disc
            all_modes = agent.decision_logic.mode_defs
            
            file_path = str(getmodule(stack()[len(stack())-1][0]).__file__) # path to the file
            directory, filename = os.path.split(file_path) # extract the directory and filename
            module_name = filename[:-3] # remove the .py extension
            spec = importlib.util.spec_from_file_location(module_name, file_path)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module) # import the module dynamically
            State = getattr(module, "State") # access the State class in the file
            #print(get_type_hints(State))

            attr = dict()
            for j in parsed_modes:
                temp = str(get_type_hints(State)[j]).split(' ')[1][1:-2] # get the attribute from State class
                attr[temp] = j
            dict1 = all_modes.copy()
            for j in dict1:
                if j not in attr:
                    all_modes.pop(j)
                else:
                    all_modes[attr[j]] = all_modes.pop(j)
            
            modes = []
            for j in all_modes:
                temp = []
                for k in agent.decision_logic.mode_defs[j].modes:
                    temp.append(j+'.'+k)
                modes.append(temp)
            controller_modes = [','.join(str(y) for y in x) for x in itertools.product(*modes)]
            for j in controller_modes:
                G.add_node(j)

            # Add a node to indicate the initial controller mode of the agent
            init_mode = ""
            for j in range(len(init_mode_list[i])):
                init_mode+=str(init_mode_list[i][j])
                if j != len(init_mode_list[i]) - 1:
                    init_mode+=","
            init_mode = init_mode.split(',')
            init_mode_final = ""
            for index in range(len(init_mode)):
                j = init_mode[index]
                k = j.split('.')
                x = parsed_modes[index]
                init_mode_final+= x+'.'+k[1]
                if index != len(init_mode) - 1:
                    init_mode_final += ','
            G.add_node("init")
            G.add_edge("init",init_mode_final,edge_id=0) # indicates the initial controller mode of the agent
                
            # Traverse every pair of controller modes, check which modepaths satisfy the transition
            # TODO: Enumerate all paths, self-loops or variable to mode transitions, mode to variable transitions
            # TODO: Bottom-up to top-down

            for j in controller_modes:
                for k in controller_modes:
                        if j == k:
                            continue # we are only interested in pairs of distinct controller modes
                        for l in paths: # traverse through all the modepaths
                            guard = unparse(l.cond_veri)
                            assignment = str(l.var + "=" + unparse(l.val_veri))
                            parent = [tuple(x.split('.')) for x in j.split(',')]
                            child = [tuple(x.split('.')) for x in k.split(',')]
                            # Find out the transition switches occuring between the two modes
                            parent_trans = list(set(parent) - set(child))
                            child_trans = list(set(child) - set(parent))
                            flag = 1
                            for i in range(len(parent_trans)):
                                if str(parent_trans[i][0] + " == '" + parent_trans[i][1] + "'") in guard:
                                    if str(child_trans[i][0] + "=" + child_trans[i][1]) in assignment:
                                        continue
                                else:
                                    flag = 0
                                    break
                            if flag == 1:
                                if not (guard,assignment) in G_edge_dict.values():
                                    G_edge_dict[G_edge_id] = (guard,assignment)
                                    G.add_edge(j,k,edge_id=G_edge_id)
                                    G_edge_id += 1
                                else:
                                    eid = list(G_edge_dict)[list(G_edge_dict.values()).index((guard,assignment))]
                                    G.add_edge(j,k,edge_id=eid)
            ...
            for l in paths:
                ...
                for j in parsed_modes:
                    if j in guard:
                        for k in all_modes[j].modes:
                            if str(j + " == '" + k + "'") in guard:
                                if str(j + "=") in assignment:
                                    parent = str(j+'.'+k)
                                    child = str(j+'.'+assignment.split('=')[1][1:-2])
                                    print(parent, child)
                                    #G.add_edge(parent,child,edge_id=G_edge_id)
            '''

            # Remove all nodes that are not reachable from the initial controller mode
            ...
            # Add modepaths that are not mode to mode transitions
            for l in paths:
                f1 = 1
                f2 = 1
                guard = unparse(l.cond_veri)
                assignment = str(l.var + "=" + unparse(l.val_veri))
                if any (str(j + " == '") in guard for j in parsed_modes):
                    f1 = 0
                if any (str(j + "=") in assignment for j in parsed_modes):
                    f2 = 0
                if f1*f2 == 0:
                    continue
                else: # add self-loop to all nodes as this modepath is available for all controller modes
                    for node in G.nodes():
                        if node == "init":
                            continue
                        if not (guard,assignment) in G_edge_dict.values():
                            G_edge_dict[G_edge_id] = (guard,assignment)
                            G.add_edge(node,node,edge_id=G_edge_id)
                            G_edge_id += 1
                        else:
                            eid = list(G_edge_dict)[list(G_edge_dict.values()).index((guard,assignment))]
                            G.add_edge(j,k,edge_id=eid)
            ...
            for e in G.edges(data=True):
                edge_id = e[2]['edge_id']
                if edge_id != 0:
                    guard, assignment = G_edge_dict[edge_id]
                    print(guard, assignment)
            '''

            # Return the controller graph along with the edge dictionary containing modepath info
            disc_transitions[agent.id] = [G, G_edge_dict]
